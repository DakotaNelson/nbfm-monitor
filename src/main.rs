...
fn main() {
    ...
    const FFT_SIZE: usize = SAMP_RATE;
    ...
    let fftbuf: [f32; FFT_SIZE];
    fftbuf = buf.iter().map(|val| {val.norm()}).collect::<Vec<f32>>().try_into().unwrap();

    let mut fftoutfile = BufWriter::new(File::create(fname).expect("error opening output file"));
    write_scalar_file(&fftbuf, &mut fftoutfile).expect("failed to write fft output");

    // TODO running average of FFT values
}
